...
async def project(interact: discord.Interaction, project: str):
    ...
    msg.add_field(name="Loves per view :", value=f"{round(project_obj.loves / project_obj.views, 2)} :heart: / :eye:")
    msg.add_field(name="Faves per view :", value=f"{round(project_obj.favorites / project_obj.views, 2)} :star: / :eye:")
    msg.add_field(name="Loves per view (%) :", value=f"{round((project_obj.loves / project_obj.views) * 100)} :heart: / 100 :eye:")
    msg.add_field(name="Faves per view (%) :", value=f"{round((project_obj.favorites / project_obj.views) * 100)} :star: / 100 :eye:")
...
async def s_download(interact: discord.Interaction, project: str):
    ...
    await interact.followup.send(file=discord.File(
        fp="ScratchOn_private/project.sb3",
        filename=f"{proj.title}.sb3"
    ))
