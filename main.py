...
async def dc2scratch(username):
    with (
        ...
        if username in dc_users:
            ...
            return sc_users[
                index
            ]  # return the corresponding scuser, no need to strip again
        else:
            return None
...
async def render_blocks_image(
    code: str, style: str, output_path="ScratchOn_private/blocks.png"
):
...
def update_pings():
    ...
    if multievents == True:
        ...
        @combined.event(
            function=combined
        )  # defines the event for all eventhandlers that were combined
        def on_ready():
            print("Message trackers are up to date !")

        @combined.request(
            function=combined
        )  # defines the request for all request handlers that were combined
        def your_request(): ...
        ...
        def on_count_change(
            old_count, new_count
        ):  # Called when the user's message count changes.
...
async def on_interaction(interaction: discord.Interaction):
    if interaction.type == discord.InteractionType.component:
        ...
        if button_id == "ai_button":
            ...
            new_button = Button(
                label="Embeds", style=new_style, custom_id="embeds_button"
            )

        # Rebuild the view with the updated button
        ai_button = Button(
            label="AI", style=button_states["ai_button"], custom_id="ai_button"
        )
        embeds_button = Button(
            label="Embeds",
            style=button_states["embeds_button"],
        ...
        await interaction.followup.send(
            f"The {button_id} color changed to {'green' if new_style == discord.ButtonStyle.green else 'red'}!",
            ephemeral=True,
        )
...
async def studio(interact: discord.Interaction, studio: str):
    ...
    msg.set_footer(
        text=f"Studio id : {studio.id}, link : https://scratch.mit.edu/studios/{studio.id}"
    )
...
async def scratchstatus(interact: discord.Interaction):
    ...
    modified_description = re.sub(
        r"'([^']+)'", r"**\1**", modified_description
    )  # Make text between single quotes bold
...
async def event(interact: discord.Interaction):
    ...
    projects = scratch.search_projects(
        query="christmas", mode="trending", language="en", limit=10, offset=0
    )
...
async def project(interact: discord.Interaction, project: str):
    ...
    msg.add_field(
        name="Loves per view :",
        value=f"{round(project.loves / project.views, 2)} :heart: / :eye:",
    )
    msg.add_field(
        name="Faves per view :",
        value=f"{round(project.favorites / project.views, 2)} :star: / :eye:",
    )
    msg.add_field(
        name="Loves per view (%) :",
        value=f"{round((project.loves / project.views) * 100)} :heart: / 100 :eye:",
    )
    msg.add_field(
        name="Faves per view (%) :",
        value=f"{round((project.favorites / project.views) * 100)} :star: / 100 :eye:",
    )
...
async def trendscore(interact: discord.Interaction, project: str):
    ...
    )  # converts weird API date to python understandable date.
    await interact.response.send_message(
        embed=discord.Embed(
            title=f"<:popular:1330550904813916272>'{proj.title}' has a trending score of {round(proj.views / (diff.days * 24 + diff.seconds / 3600), 3)} !<:popular:1330550904813916272>",
...
@bot.tree.command(
    name="randomprojects",
    description="Shows a number of random scratch projects, powered by ESDB.",
)
async def randomprojects(interact: discord.Interaction, number: int):
    response = requests.get(
        f"https://explore.eletrix.fr/api/projects/random?limit={number}"
    )
...
@bot.tree.command(
    name="bettersearch",
    description="Allows you to search projects without bugs such as <900 million ids. Powered by ESDB.",
)
async def bettersearch(interact: discord.Interaction, query: str):
    response = requests.get(
        f"https://explore.eletrix.fr/api/projects/search?search={query}"
    )
    if response.status_code == 200:
        message = ""
        for item in response.json():
...
async def s_download(interact: discord.Interaction, project: str):
    ...
    await interact.followup.send(
        file=discord.File(
            fp="ScratchOn_private/project.sb3", filename=project.title().sb3
        )
    )
...
async def recommend(interact: discord.Interaction, type: str):
    if interact.user.name == "fluffygamer.":
        ...
        if not scratch_username == None:
            ...
            interact.response.send_message(
                embed=discord.Embed(
